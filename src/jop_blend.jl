...
function JopBlend(T, nsamples, shottimes, nsamples_blended=0)
    function df!(d::AbstractArray{T,1}, m::AbstractArray{T,2}; st, kwargs...) where {T}
        d .= 0
    ...
    function df′!(m::AbstractArray{T,2}, d::AbstractArray{T,1}; st, kwargs...) where {T}
        m .= 0
        nsamples, nshots = size(m)
        nsamples_blended = length(d)
        for ishot = 1:nshots, isample = 1:nsamples
    ...
    JopLn(df! = df!, df′! = df′!, dom = dom, rng = rng, s = (st=shottimes,))
end
export JopBlend
